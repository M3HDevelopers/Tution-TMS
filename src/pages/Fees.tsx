...
export default function Fees() {
  ...
  const saveWaive = () => {
    ...
    if (!rec.waived && waiveReason.trim().length < 3) {
      setWaiveErr("A reason is required — e.g. \"Family emergency, fee waived for this month.\"");
      return;
    }
    ...
    patch({
      ...
      activity: withActivity(next, removing
        ? `Waiver removed — ${studentName} (${periodLabel(rec.period)}); fee is due again.`
        : `${periodLabel(rec.period)} fee waived — ${studentName}. Reason: "${waiveReason.trim()}".`, "fee"),
    });
    toast.push(removing ? "Waiver removed — fee is due again" : `${periodLabel(rec.period)} fee waived`);
  ...
  return (
    <div>
                ...
                <div key={s.id} className={`card overflow-hidden transition-colors ${isOpen ? "border-gold-500/40 shadow-[var(--shadow-lift)]" : ""}`}>
                  ...
                  <div className="flex items-center gap-2 px-4 py-3">
                    ...
                    <Btn size="sm" variant={owesAny ? "gold" : "outline"} icon="wallet" className={owesAny ? "" : "opacity-45"} onClick={() => {
                      if (!owesAny) { toast.push(`${s.name} has no pending dues — nothing to collect.`, "warn"); return; }
                      ui.openPayment(s.id);
                    }}>Pay</Btn>
                        ...
                        <div>
                          ...
                          {rec ? (
                            <div className="rounded-[10px] border border-ink-150 bg-white divide-y divide-ink-100 text-[12.5px]">
                              ...
                              {rec.waived
                                ? <Row k="Waived ✓" v={rec.waiveReason || "fee forgiven"} green />
                                : bal > 0
                          ...
                          <div className="flex flex-col w-full gap-2 mt-3 [&>*]:w-full sm:flex-row sm:items-center sm:[&>*]:w-auto">
                            ...
                            {rec && !rec.waived && bal > 0 && <Btn size="sm" variant="outline" onClick={() => { setWaiveRec(rec.id); setWaiveReason(""); }}>Waive Fee</Btn>}
                            {rec && rec.waived && <Btn size="sm" variant="outline" icon="refresh" onClick={() => setWaiveRec(rec.id)}>Undo Waiver</Btn>}
      ...
      {/* Waive fee modal — a reason is required */}
      <Modal open={!!waiveRec} onClose={() => { setWaiveRec(null); setWaiveErr(""); }}
        title={state.feeRecords.find((r) => r.id === waiveRec)?.waived ? "Undo Fee Waiver" : "Waive This Month's Fee"}
        sub={(() => {
        ...
        footer={<>
          ...
          {state.feeRecords.find((r) => r.id === waiveRec)?.waived
            ? <Btn variant="danger" icon="refresh" onClick={saveWaive}>Yes, Undo Waiver</Btn>
            : <Btn variant="gold" icon="check" onClick={saveWaive}>Waive Fee</Btn>}
        </>}>
        {state.feeRecords.find((r) => r.id === waiveRec)?.waived ? (
          <p className="text-[13px] text-ink-600 leading-relaxed">
            This month's fee was previously waived{state.feeRecords.find((r) => r.id === waiveRec)?.waiveReason ? <> with the reason: <b>"{state.feeRecords.find((r) => r.id === waiveRec)?.waiveReason}"</b></> : null}. Undoing the waiver will make this fee <b>due again</b> and it will be added back to the outstanding balance.
          </p>
        ) : (
          <div className="space-y-3">
            <p className="text-[13px] text-ink-600 leading-relaxed">
              Only the fee for <b>{periodLabel(state.feeRecords.find((r) => r.id === waiveRec)?.period ?? "")}</b> will be waived — other months are unaffected. Payments already received stay safe in history.
            </p>
            <label className="block">
              ...
              <textarea value={waiveReason} onChange={(e) => { setWaiveReason(e.target.value); setWaiveErr(""); }} rows={3}
                placeholder="e.g. Family emergency — fee waived for this month"
