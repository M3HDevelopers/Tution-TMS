...
import type { FeeStatus, StudentStatus, AttendanceStatus } from "../types";
...
const PATHS: Record<string, React.ReactNode> = {
  ...
  batch: <><path d="M4 7.5 12 4l8 3.5-8 3.5z" /><path d="M6.5 9.6V15c0 1.4 2.5 2.8 5.5 2.8s5.5-1.4 5.5-2.8V9.6" /><path d="M20 8.5v5" /></>,
  ...
  settings: <><circle cx="12" cy="12" r="3" /><path d="M12 2.8l1.2 2.4 2.7.5 2 2-2 2.3.4 2.7-2.3 1.6-2.6-.9-2.6.9-2.3-1.6.4-2.7-2-2.3 2-2 2.7-.5z" /></>,
  ...
  restore: <><path d="M3.5 8A9 9 0 1 1 3 13.5" /><path d="M3.5 3.5V8H8" /></>,
...
export function FeeStatusBadge({ status, pulse }: { status: FeeStatus; pulse?: boolean }) {
  const tone: Tone = status === "paid" ? "green" : status === "overdue" ? "red" : status === "partial" ? "amber" : status === "waived" ? "slate" : status === "upcoming" ? "navy" : "gold";
  return <Badge tone={tone} className={pulse && status === "overdue" ? "pulse-dot" : ""}>{FEE_STATUS_LABEL[status]}</Badge>;
}

export function StudentStatusBadge({ status }: { status: StudentStatus }) {
  return <Badge tone={status === "active" ? "green" : status === "inactive" ? "amber" : "slate"}>{status === "active" ? "Active" : status === "inactive" ? "Inactive" : "Archived"}</Badge>;
}
...
export function AttBadge({ status }: { status: AttendanceStatus | "notmarked" | "holiday" }) {
  const map: Record<string, [Tone, string]> = {
    ...
    late: ["amber", "Late"],
    leave: ["slate", "Leave"],
    notmarked: ["slate", "Not Marked"],
    holiday: ["teal", "Holiday"],
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6" role="dialog" aria-modal="true">
      ...
      <div className={`relative w-full ${wide ? "sm:max-w-3xl" : "sm:max-w-lg"} max-h-[92vh] flex flex-col bg-white sm:rounded-2xl rounded-t-2xl border border-ink-100 shadow-2xl anim-pop`}>
        ...
        {footer && <div className="px-6 py-4 border-t border-ink-100 bg-ink-50/60 rounded-b-2xl flex items-center justify-end gap-2.5 flex-wrap">{footer}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <ToastCtx.Provider value={{ push }}>
      ...
      <div className="fixed bottom-5 right-5 z-[70] flex flex-col gap-2 items-end pointer-events-none">
        {items.map((t) => (
          <div key={t.id} className={`anim-toast pointer-events-auto flex items-center gap-2.5 pl-3 pr-4 h-11 rounded-[10px] border shadow-lg text-[13px] font-semibold bg-ink-900 text-white ${t.tone === "ok" ? "border-ink-700" : t.tone === "warn" ? "border-gold-600/50" : "border-flame-600/50"}`}>
...
export function Stat({ label, value, sub, icon, tone = "navy", onClick, delay }: { label: string; value: React.ReactNode; sub?: React.ReactNode; icon: string; tone?: Tone; onClick?: () => void; delay?: number }) {
  ...
  return (
    <El
      ...
      className={`card p-4 text-left anim-fade-up ${onClick ? "card-hover cursor-pointer w-full" : ""}`}
    >
